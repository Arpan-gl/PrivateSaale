...
import { demoSignUp, isDemoMode, setDemoMode } from '../services/demoAuth';
...
const SignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (demoMode) {
        // Use demo signup
        response = await demoSignUp(signupData);
        
        if (response.success) {
          // Auto-login in demo mode
          dispatch(login({ user: response.user, isDemoMode: true }));
          toast({
            title: "Success",
        ...
        // Use real API
        const apiResponse = await axios.post('/signUp', signupData);
  ...
  const toggleDemoMode = () => {
    ...
    if (newDemoMode) {
      toast({
        ...
        description: "You can now test the application with demo data",
        variant: "default",
      });
    } else {
      toast({
          ...
          {demoMode && (
            <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
              <div className="text-sm text-blue-800 text-center">
                <strong>Demo Mode:</strong> In demo mode, you'll be automatically logged in after registration
              </div>
            </div>
          )}
        </CardHeader>
