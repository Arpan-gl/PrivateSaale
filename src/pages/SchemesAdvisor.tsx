...
import { DemoService, DemoScheme } from '@/services/demoData';
...
export default function SchemesAdvisor() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const schemes = await DemoService.getSchemeRecommendations(form);
      
      // Format the schemes as a text response similar to the original API
      const formattedResult = schemes.map((scheme, index) => 
        `${index + 1}. ${scheme.name}\n   Why it matches: ${scheme.description}\n   Key eligibility: ${scheme.eligibility.join(', ')}\n   Benefit summary: ${scheme.benefits.join(', ')}`
      ).join('\n\n');
      
      setResult(formattedResult || 'No matching schemes found for your profile.');
    } catch (err: any) {
