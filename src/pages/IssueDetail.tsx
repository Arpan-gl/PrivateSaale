...
import { DemoService, demoUser } from '../services/demoData';
...
const IssueDetail: React.FC = () => {
    ...
    try {
      if (isDemoMode()) {
        ...
        setUser({
          ...
          role: demoUser.role,
          lawyerProfile: {
            isVerified: demoUser.role === 'lawyer'
          }
        });
      } else {
  ...
  const fetchIssue = async (issueId: string) => {
    try {
      ...
      if (isDemoMode()) {
        // Use demo data
        const issue = await DemoService.getIssueById(issueId);
        if (issue) {
          setIssue(issue);
          // Increment views
          await DemoService.incrementViews(issueId);
        } else {
  ...
  const fetchResponses = async (issueId: string) => {
    try {
      ...
      if (isDemoMode()) {
        ...
        const issue = await DemoService.getIssueById(issueId);
        if (issue) {
          setResponses(issue.responses.map(response => ({
            _id: response._id,
            issue: issueId,
            lawyer: {
              _id: response.lawyer._id,
              username: response.lawyer.name,
              email: response.lawyer.email,
              lawyerProfile: { isVerified: response.lawyer.isVerified }
            },
            content: response.content,
            approach: 'Professional legal analysis and strategic approach',
            ...
            qualifications: ['Bar Certified', '10+ Years Experience'],
            experience: {
              years: response.lawyer.experience_years,
              description: response.lawyer.bio
            },
