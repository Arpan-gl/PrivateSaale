...
import { DemoService, DemoIssue } from '@/services/demoData';
...
const LegalIssues: React.FC<LegalIssuesProps> = ({ onPostIssue, onViewIssue }) => {
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      // Use demo service to get issues
      const demoIssues = await DemoService.getIssues();
      ...
      if (selectedStatus && selectedStatus !== 'All') {
        ...
        filteredIssues = filteredIssues.filter(issue => 
          issue.status === statusMap[selectedStatus]
        );
      }
      ...
      setTotalPages(1); // Demo data doesn't need pagination
    } catch (error: any) {
