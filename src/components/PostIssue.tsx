    ...
    try {
      ...
      if (isDemoMode()) {
        ...
        const issueData = {
          ...
          urgency: formData.urgency.toLowerCase() as 'low' | 'medium' | 'high',
          status: 'open' as const
        };
        
        response = await DemoService.createIssue(issueData);
        
        toast({
          title: 'Success',
          description: 'Legal issue posted successfully! (Demo Mode)',
          variant: 'default',
        });
      } else {
        // Use real API
        response = await axios.post('/issues', formData);
        ...
        if (response.data.success) {
          ...
          toast({
            title: 'Error',
            description: response.data.message || 'Failed to post issue',
            variant: 'destructive',
          });
          return;
        }
