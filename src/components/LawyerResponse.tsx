...
import { DemoService, demoLawyers } from '../services/demoData';
...
const LawyerResponse: React.FC<LawyerResponseProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isDemoMode()) {
        // Use demo service
        const lawyerId = demoLawyers[0]._id; // Use first demo lawyer
        response = await DemoService.respondToIssue(issueId, formData.content, lawyerId);
        
        toast({
          title: 'Success',
          description: 'Your response has been posted successfully! (Demo Mode)',
          variant: 'default',
        });
      } else {
        // Use real API
        response = await axios.post('/responses', {
          issueId,
          ...formData
        });
        ...
        if (response.data.success) {
          ...
          toast({
            title: 'Error',
            description: response.data.message || 'Failed to post response',
            variant: 'destructive',
          });
          return;
        }
