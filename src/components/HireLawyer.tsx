...
const HireLawyer: React.FC<HireLawyerProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isDemoMode()) {
        // Use demo service
        hireResponse = await DemoService.hireLawyer(response.lawyerId, issue._id);
        
        toast({
          title: 'Success',
          description: 'Lawyer hired successfully! (Demo Mode) You can now communicate and track your case.',
          variant: 'default',
        });
      } else {
        // Use real API
        ...
        hireResponse = await axios.post('/hire', {
        ...
        if (hireResponse.data.success) {
          ...
          toast({
            title: 'Error',
            description: hireResponse.data.message || 'Failed to hire lawyer',
            variant: 'destructive',
          });
          return;
        }
